...
from .serializers import AssignmentSerializer, ClassRoomSerializer, AddStudentSerializer, QuestionSerializer, SubmissionSerializer, SubmitPayloadSerializer
...
class ClassRoomViewSet(viewsets.ModelViewSet):
    ...
    def grade_submission(self, request, pk=None, sid=None):
        ...
        # Only allow grading submissions for this class
        subs = Submission.objects.filter(classroom=classroom).select_related("student").prefetch_related("answers__question__choices")
        if not sub:
            return Response({"detail": "Submission not found."}, status=status.HTTP_404_NOT_FOUND)

        answers_map = request.data.get("answers", {})
        if not isinstance(answers_map, dict):
            return Response({"detail": "answers must be an object map: {answer_id: points}"},
                            status=status.HTTP_400_BAD_REQUEST)

        # Apply points to SHORT answers only (MVP)
        for ans in sub.answers.all():
            ...
            try:
                ...
                return Response({"detail": f"Invalid points for answer {ans.id}"},
                                status=status.HTTP_400_BAD_REQUEST)

            ans.points = max(0, pts)
            ans.is_correct = None  # still subjective; keep None
            ans.save()

        # Recompute total score + mark graded
...
class StudentClassRoomViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def submit(self, request, pk=None):
        ...
        print("SUBMIT answers type:", type(raw), "value:", raw)
        ...
        if isinstance(raw, dict):
            ...
            for item in raw:
                if isinstance(item, dict) and "question" in item and "response" in item:
                    ...
                    return Response(
                        {"detail": "Invalid answers list format. Use {question, response} objects."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
        else:
            return Response(
                {"detail": "answers must be an object (dict) or a list."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        Submission.objects.filter(classroom=classroom, student=request.user).delete()

        sub = Submission.objects.create(classroom=classroom, student=request.user)

        qs = Question.objects.filter(classroom=classroom).prefetch_related("choices")
    ...
    def my_submission(self, request, pk=None):
        ...
        sub = (
            Submission.objects.filter(classroom=classroom, student=request.user)
        ...
        print("MY-SUBMISSION user:", request.user.id, request.user.username)
        print(
            "MY-SUBMISSION count:",
            Submission.objects.filter(classroom=classroom, student=request.user).count(),
        )
    ...
    def submit_assignment(self, request, pk=None, aid=None):
        ...
        if not answers_dict:
            return Response({"detail": "answers cannot be empty."}, status=status.HTTP_400_BAD_REQUEST)

        # one submission per student per assignment (MVP)
        Submission.objects.filter(classroom=classroom, student=request.user, assignment=assignment).delete()
        ...
        qs = Question.objects.filter(classroom=classroom, assignment=assignment).prefetch_related("choices")
        qmap = {str(q.id): q for q in qs}
